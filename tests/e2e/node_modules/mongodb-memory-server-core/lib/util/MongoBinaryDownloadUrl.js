...
class MongoBinaryDownloadUrl {
    ...
    getSuseVersionString(os) {
        ...
        if (suseRelease === 13 || suseRelease === 14) {
            throw new errors_1.KnownVersionIncompatibilityError(`${os.dist} ${suseRelease || os.release || os.codename}`, this.version, 'none', 'Mongodb does not provide binaries for Suse version 13 & 14');
        }
        ...
        if (suseRelease < 15 &&
            ...
            throw new errors_1.KnownVersionIncompatibilityError(`${os.dist} ${suseRelease || os.release || os.codename}`, this.version, '<8.0.0', 'Mongodb only provides binaries for suse 15 for 8.0.0 and above');
        }
        // base case for higher than mongodb supported suse versions or no version is available
        {
            ...
            if (suseRelease > highestSuseRelease) {
                log(`getSuseVersionString: suseRelease "${suseRelease}" is higher than the currently supported mongodb release of "${highestSuseRelease}". Using highest known suse release.`);
    ...
    getUbuntuVersionString(os) {
        ...
        // base case for higher than mongodb supported ubuntu versions
        {
            ...
            if (ubuntuYear > highestUbuntuYear) {
                log(`getUbuntuVersionString: ubuntuYear "${ubuntuYear}" is higher than the currently supported mongodb year of "${highestUbuntuYear}". Using highest known ubuntu year.`);
                return 'ubuntu2404';
            }
        }
