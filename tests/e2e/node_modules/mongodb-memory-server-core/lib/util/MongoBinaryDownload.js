...
class MongoBinaryDownload {
    ...
    async download(downloadUrl) {
        ...
        const downloadedFile = await this.httpDownload(urlObject, requestOptions, downloadLocation, tempDownloadLocation).catch((err) => {
            log(`Error:`, err);
            throw err;
        });
     ...
     */
    async httpDownload(url, httpOptions, downloadLocation, tempDownloadLocation, maxRetries, baseDelay = 1000, timeout = 60000) {
        log('httpDownload');
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                return await this.attemptDownload(url, useHttpsOptions, downloadLocation, tempDownloadLocation, downloadUrl, httpOptions, timeout);
            }
            catch (error) {
                ...
                if (!shouldRetry || attempt === retries) {
                    if (error instanceof errors_1.DownloadError) {
        ...
        const error = new errors_1.DownloadError(downloadUrl, 'Max retries exceeded', 'Max retries exceeded');
        error.maxRetries = retries;
     ...
     * @private
     */
    async attemptDownload(url, useHttpsOptions, downloadLocation, tempDownloadLocation, downloadUrl, httpOptions, timeout = 60000) {
        /** Offset to resume from; for now a non-0 value indicates to use file "append" mode */
        let offset = 0;
        if ((0, resolveConfig_1.envToBool)((0, resolveConfig_1.default)(resolveConfig_1.ResolveConfigVariables.EXP_RESUME_DOWNLOAD))) {
            const stat = await (0, utils_1.statPath)(tempDownloadLocation);
            if (stat && stat.size != 0) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const request = follow_redirects_1.https.get(url, useHttpsOptions, async (response) => {
                if (response.statusCode != 200 && response.statusCode != 206) {
                    if (response.statusCode === 403) {
                        reject(new errors_1.DownloadError(downloadUrl, "Status Code is 403 (MongoDB's 404)\n" +
                            ...
                            'https://www.mongodb.com/download-center/community/releases/archive', 'Status: 403'));
                        return;
                    }
                    // delete temporary file on "416: Range Not Satisfiable" and let it re-try
                    if (response.statusCode === 416) {
                        await fs_1.promises.rm(tempDownloadLocation, { force: true });
                    }
                    reject(new errors_1.DownloadError(downloadUrl, `Status Code isn't 200! (it is ${response.statusCode})`, `Status: ${response.statusCode}`));
                    return;
                }
                ...
                if (!(0, resolveConfig_1.envToBool)((0, resolveConfig_1.default)(resolveConfig_1.ResolveConfigVariables.DOWNLOAD_IGNORE_MISSING_HEADER)) &&
                    contentLength <= 0) {
                    reject(new errors_1.DownloadError(downloadUrl, 'Response header "content-length" does not exist or resolved to NaN', 'Invalid Content-Length'));
                    return;
                }
                ...
                const fileStream = (0, fs_1.createWriteStream)(tempDownloadLocation, { /* start: offset, */ flags });
                fileStream.on('error', (err) => {
                    response.destroy();
                    // use the code if available, otherwise use the entire message
                    const code = err?.code ?? err.message;
                    reject(new errors_1.DownloadError(downloadUrl, err.message, code));
                });
                response.pipe(fileStream);
                fileStream.on('finish', async () => {
                    if (this.dlProgress.current < this.dlProgress.length &&
                        !httpOptions.path?.endsWith('.md5')) {
                        reject(new errors_1.DownloadError(downloadUrl, `Too small (${this.dlProgress.current} bytes) mongod binary downloaded.`, 'File too small'));
                        return;
                    }
            ...
            request.setTimeout(timeout, () => {
                request.destroy();
                reject(new errors_1.DownloadError(downloadUrl, `Request timeout after ${timeout}ms`, 'ETIMEDOUT'));
            });
        });
    }
    /**
